...
static void advance() {
    ...
    for (;;) {
        ...
        // printf("token is %s\n", TOKEN_NAMES[parser.current.type]);
...
static int resolveLocal(Compiler* compiler, Token* name) {
    for (int i = compiler->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            if (local->depth == -1) {
                errorAtCurrent("Can't read local var in its own initalizer.");
            }
            return i;
        }
    }
...
static int addUpvalue(Compiler* compiler, uint8_t index,
    ...
    if (upvalueCount == UINT8_COUNT) {
        errorAtCurrent("Too many closure variables in fucntion.");
        return 0;
    }
...
static void declareVariable() {
    ...
    for(int i = current->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            errorAtCurrent(
                    "Already varialbe with this name in this scope.");
        }
    }
    addLocal(*name);
}
...
static uint8_t argumentList() {
    ...
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
            expression();
            if (argCount == 255) {
                errorAtCurrent("Can't have more than 255 args.");
            }
            argCount++;
...
static void super_(bool canAssign) {
    ...
    consume(TOKEN_IDENTIFIER, "Expect superclass method name super'.");
    uint8_t name = identifierConstant(&parser.previous);
...
static void forStatement() {
    ...
    if (!match(TOKEN_SEMICOLON)) {
        ...
        // jump out if conditon is not true
